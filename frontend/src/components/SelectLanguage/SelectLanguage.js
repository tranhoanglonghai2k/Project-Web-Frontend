...
export const SelectLanguage = () => {
    ...
    const check = localStorage.getItem("his") ?  JSON.stringify(localStorage.getItem("his")) : [] ;
    const [his,setHis] = useState(check); 
      ...
      function handleSubmit(event) {
        ...
        axios.post(END_POINT + "/api/translate-paragraph", {"type": request_lang, "param": input })
        .then(res => {
          console.log(res);
        ...
        return (
          ...
          <div className="Container-language">
              <select className="language-input" name ="lang1" value={lang1} onChange={handleChangelang1}>
          ...
          <div className="container-translate">
            <div><textarea name="input" onChange={handleChangeinput} /></div>
            <div >{output}</div>
          </div> 
